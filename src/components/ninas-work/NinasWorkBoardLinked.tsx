...
function employeeLabel(employee: EmployeeRecord | undefined) {
    return employee ? `${employee.username}` : "Unbekannt";
}
...
export default function NinasWorkBoardLinked({
    ...
    const [activeTab, setActiveTab] = useState<"tasks" | "care">("tasks");
    ...
    useEffect(() => {
        ...
        setMessageForm((current) => ({ text: current.text }));
    }, [board.tasks, currentEmployeeId, selectedTaskId]);
    ...
    const createCareTask = async () => {
        const response = await fetch("/api/ninas-work/care", {
            ...
            body: JSON.stringify({
                ...careForm,
                dischargedAt: careForm.dischargedAt || null,
            }),
        });
            ...
            <div className={`${nw.panelPadded} shadow-lg shadow-slate-400/20 sm:p-8`}>
                <div className="flex flex-wrap items-end justify-between gap-4">
                    <div>
                        ...
                        <p className={nw.compactTitle}>
                            Eingeloggt als: {currentEmployeeName}
                        </p>
                    </div>
            ...
            <div className={nw.panelPadded}>
                <div className="flex items-center justify-between gap-4">
                    ...
                    <button
                        type="button"
                        onClick={createTask}
                        className={`${nw.buttonPrimaryMd} px-4 py-2`}
                    >
                        Speichern
                    </button>
                </div>
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                    {isAdmin ? (
                        <label className="space-y-2">
                            ...
                            <select
                                value={createForm.creatorEmployeeId}
                                onChange={(event) =>
                                    setCreateForm((current) => ({
                                        ...current,
                                        creatorEmployeeId: Number(event.target.value),
                                    }))
                                }
                                className={nw.input}
                            >
                    ...
                    <label className="space-y-2">
                        ...
                        <select
                            value={createForm.assigneeEmployeeId ?? ""}
                            onChange={(event) =>
                                setCreateForm((current) => ({
                                    ...current,
                                    assigneeEmployeeId: event.target.value ? Number(event.target.value) : null,
                                }))
                            }
                            className={cx(nw.input, !createForm.assigneeEmployeeId && nw.assigneeMissing)}
                        >
                ...
                <div className={nw.panelPadded}>
                    <div className="flex items-center justify-between gap-4">
                        <div>
                            ...
                            <p className={`mt-2 text-sm ${nw.textMuted}`}>
                                Änderungen werden auf zukünftige, nicht manuell angepasste Subtasks übertragen. Es werden immer 14 Tage voraus geplant.
                            </p>
                        </div>
                        <button type="button" onClick={createMasterTask} className={`${nw.buttonPrimaryMd} px-4 py-2`}>
                            Mastertask speichern
                        </button>
                    </div>
                    ...
                    <div className="mt-5 grid gap-4 sm:grid-cols-2">
                        <label className="space-y-2">
                            <span className={nw.compactTitle}>Ersteller</span>
                            <select
                                value={masterForm.creatorEmployeeId}
                                onChange={(event) =>
                                    setMasterForm((current) => ({
                                        ...current,
                                        creatorEmployeeId: Number(event.target.value),
                        ...
                        <label className="space-y-2">
                            <span className={nw.compactTitle}>Bearbeiter</span>
                            <select
                                value={masterForm.assigneeEmployeeId ?? ""}
                                onChange={(event) =>
                                    setMasterForm((current) => ({
                                        ...current,
                                        assigneeEmployeeId: event.target.value ? Number(event.target.value) : null,
                        ...
                        <input
                            ...
                            onChange={(event) => setMasterForm((current) => ({ ...current, area: event.target.value }))}
                            placeholder="Bereich"
                            className={nw.textInput}
                        />
                        <input
                            value={masterForm.task}
                            onChange={(event) => setMasterForm((current) => ({ ...current, task: event.target.value }))}
                            placeholder="Mastertask"
                            className={nw.textInput}
                        />
                        ...
                        <label className="space-y-2">
                            ...
                            <input
                                type="number"
                                min={1}
                                value={masterForm.frequencyDays}
                                onChange={(event) => setMasterForm((current) => ({ ...current, frequencyDays: Number(event.target.value) || 1 }))}
                                className={nw.input}
                            />
                        </label>
                        ...
                        <label className="space-y-2">
                            ...
                            <input
                                ...
                                onChange={(event) => setMasterForm((current) => ({ ...current, startDate: event.target.value }))}
                                className={nw.input}
                            />
                        </label>
                    </div>
                            ...
                            <article key={masterTask.id} className={nw.listCard}>
                                <div className="flex flex-wrap items-center justify-between gap-3">
                                    <div>
                                        ...
                                        <p className={`mt-1 text-sm ${nw.textMuted}`}>
                                            Bereich: {masterTask.area} | Bearbeiter: {assigneeLabel(masterTask.assigneeUsername)}
                                        </p>
                                    </div>

                                    <div className="flex flex-wrap gap-2">
                                        <button type="button" onClick={() => quickEditMasterTask(masterTask)} className={nw.linkButton}>
                                            Bearbeiten
                                        </button>
                                        <button type="button" onClick={() => toggleMasterTaskActive(masterTask)} className={nw.linkButton}>
                                            {masterTask.active ? "Pausieren" : "Aktivieren"}
                                        </button>
                                        <button type="button" onClick={() => deleteMasterTask(masterTask)} className={nw.buttonDanger}>
                                            Löschen
                                        </button>
                                    </div>
                                </div>
            ...
            <div className="space-y-3">
                {board.tasks.map((task) => {
                    ...
                    return (
                        <article
                            key={task.id}
                            className={expanded ? nw.taskCardExpanded : nw.taskCard}
                        >
                            <div
                                role="button"
                                tabIndex={0}
                                ...
                                <div className="flex flex-wrap items-start justify-between gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <div className="mt-1 flex items-center gap-2">
                                            ...
                                            <span
                                                className={cx(nw.assigneeBadge, task.assigneeEmployeeId ? nw.assigneeOk : nw.assigneeMissing)}
                                            >
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <svg
                                            className={`h-5 w-5 text-slate-400 transition-transform duration-200 ${expanded ? "rotate-180" : "rotate-0"}`}
                                            viewBox="0 0 20 20"
                                            fill="none"
                                            aria-hidden="true"
                                        >
                                        ...
                                        <span className="sr-only">{expanded ? "Zuklappen" : "Aufklappen"}</span>
                                    </div>
                                </div>
                            </div>

                            {expanded ? (
                                <div className="space-y-5 border-t border-[color:var(--nw-border)] p-5">
                                    <div className="grid gap-4 sm:grid-cols-2">
                                        ...
                                        <input
                                            value={editForm.area}
                                            onChange={(event) => markEditField("area", event.target.value)}
                                            onBlur={() => clearEditField("area")}
                                            placeholder="Bereich"
                                            className={cx(nw.textInput, editingFields.area && nw.fieldEditing)}
                                        />
                                        <input
                                            value={editForm.task}
                                            ...
                                            placeholder="Aufgabe"
                                            className={cx(nw.textInput, editingFields.task && nw.fieldEditing)}
                                        />
                                        ...
                                        <label className="space-y-2 sm:col-span-2">
                                            ...
                                            <input
                                                ...
                                                onChange={(event) => markEditField("dueDate", event.target.value)}
                                                onBlur={() => clearEditField("dueDate")}
                                                className={cx(nw.input, editingFields.dueDate && nw.fieldEditing)}
                                            />
                                        </label>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        <button
                                            ...
                                            className={`rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.25em] ${editForm.done
                                                ? nw.stateDone
                                                : nw.stateOpen
                                                }`}
                                        >
                                        ...
                                        <button
                                            ...
                                            className={cx(nw.buttonPrimary, !hasUnsavedChanges && nw.buttonDisabled)}
                                        >
                                            Speichern
                                        </button>
                                        <button
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex items-center justify-between gap-4">
                                            ...
                                            <span className={nw.idBadge}>
                                                {task.thread.length} Nachrichten
                                            </span>
                                        </div>
                                        ...
                                        {task.thread.map((message) => (
                                            <div key={message.id} className={nw.listCard}>
                                                <div className={`flex items-center justify-between gap-3 text-xs ${nw.textMuted}`}>
                                                    <span className="font-semibold uppercase tracking-[0.25em] text-[color:var(--nw-accent)]">
                                        ...
                                        <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
                                            ...
                                            <button
                                                ...
                                                className={`${nw.buttonPrimaryMd} self-end rounded-2xl px-5 py-3`}
                                            >
                                                Senden
                                            </button>
                                        </div>
                ...
                <section className="space-y-6">
                    <div className={nw.panelPadded}>
                        <div className="flex items-center justify-between gap-4">
                            ...
                            <button type="button" onClick={createCareTask} className={`${nw.buttonPrimaryMd} px-4 py-2`}>
                                Eintrag speichern
                            </button>
                        </div>
                        ...
                        <div className="mt-5 grid gap-4 sm:grid-cols-2">
                            <input
                                value={careForm.birdName}
                                onChange={(event) => setCareForm((current) => ({ ...current, birdName: event.target.value }))}
                                placeholder="Name des Vogels"
                                className={nw.textInput}
                            />
                            <input
                                value={careForm.careType}
                                onChange={(event) => setCareForm((current) => ({ ...current, careType: event.target.value }))}
                                placeholder="Medizin, Futter, etc."
                                className={nw.textInput}
                            />
                            <label className="space-y-2">
                                ...
                                <input
                                    type="date"
                                    value={careForm.admittedAt}
                                    onChange={(event) => setCareForm((current) => ({ ...current, admittedAt: event.target.value }))}
                                    className={nw.input}
                                />
                            </label>
                            <label className="space-y-2">
                                ...
                                <input
                                    type="date"
                                    value={careForm.dischargedAt}
                                    onChange={(event) => setCareForm((current) => ({ ...current, dischargedAt: event.target.value }))}
                                    className={nw.input}
                                />
                            </label>
                            <label className="space-y-2 sm:col-span-2">
                                ...
                                <input
                                    ...
                                    accept="image/*"
                                    onChange={(event) => {
                                        void setCareImageFromFile(event.target.files?.[0] ?? null);
                                    }}
                                    className={nw.input}
                                />
                            </label>
                            ...
                            <article key={careTask.id} className={nw.listCard}>
                                <div className="flex flex-wrap items-start justify-between gap-4">
                                    <div className="flex gap-4">
                                        {careTask.birdImage ? (
                                            <img
                                                src={careTask.birdImage}
                                                alt={careTask.birdName}
                                        ...
                                        <div>
                                            <p className={nw.taskMeta}>
                                                ...
                                                <span>
                                                    {careTask.dischargedAt ? `Entlassen: ${careTask.dischargedAt}` : "Noch in Pflege"}
                                                </span>
                                            </p>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        <button type="button" onClick={() => markCareDischarged(careTask)} className={nw.linkButton}>
                                            {careTask.dischargedAt ? "Wieder aktiv" : "Entlassen"}
                                        </button>
                                        <button type="button" onClick={() => deleteCareTask(careTask)} className={nw.buttonDanger}>
                                            Löschen
                                        </button>
                                    </div>
                                </div>
